...
describe("parseArgs", () => {
  ...
  it("returns error when only pipeline name provided", () => {
    const result = parseArgs(["plan-cycle"]);
    expect(result.ok).toBe(false);
  ...
  it("parses pipeline name and workspace", () => {
    ...
    if (result.ok) {
      expect(result.value.pipelineName).toBe("plan-cycle");
  ...
  it("parses --input flag", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--input", "Add tests"]);
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.input).toBe("Add tests");
  });

  it("parses --plan flag", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--plan", "plans/feature.md"]);
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.planPath).toBe("plans/feature.md");
  });

  it("returns error when --plan flag has no value", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--plan"]);
    expect(result.ok).toBe(false);
  ...
  it("returns error when --input value starts with --", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--input", "--other"]);
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error.message).toContain("--input flag requires a value");
  });

  it("parses --profile flag", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--profile", "copilot-default"]);
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value.profileName).toBe("copilot-default");
  });

  it("returns error when --profile flag has no value", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--profile"]);
    expect(result.ok).toBe(false);
  ...
  it("returns error when --profile value starts with --", () => {
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--profile", "--other"]);
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error.message).toContain("--profile flag requires a value");
  });
  ...
  it("ignores a --language flag as a harmless no-op (removed knob)", () => {
    // --language was removed along with the LanguageName/Language: directive
    // machinery -- toolchain routing is now derived entirely from the
    // workspace's devShell palette. Passing --language must not error; it is
    // simply not read by any known flag parser and has no effect.
    const result = parseArgs(["plan-cycle", "/tmp/ws", "--language", "typescript"]);
    expect(result.ok).toBe(true);
    if (result.ok) expect(result.value).not.toHaveProperty("language");
  });
